...
! STERAO VER_USER
INTEGER :: JL            ! Counter for the number of bubles
REAL    :: ZSHIFT        ! Used to initialize 3 warm bubles
REAL    :: ZCENTERX_AREA ! True domain center
REAL    :: ZCENTERY_AREA ! True domain center
! STERAO VER_USER
!!
!
...
!! La perturbation est constituee de 3 bulles chaudes (+1C), alignees selon un axe NW-SE
!
    ZCENTERX=(XXHAT_ll(1)+XXHAT_ll(IIU_ll))*0.5
    ZCENTERY=(XYHAT_ll(1)+XYHAT_ll(IJU_ll))*0.5
    ZCENTERX_AREA = ZCENTERX
    ZCENTERY_AREA = ZCENTERY

    ZSHIFT = 20000./SQRT(2.0)    ! Decalage de 20 km de part et d'autre du 
    ZSHIFT = 14000.              ! centre du domaine de simulation
    DO JL = -2, 0 ! STERAO  !
      ZCENTERX = ZCENTERX_AREA + ZSHIFT*REAL(JL)
      ZCENTERY = ZCENTERY_AREA - ZSHIFT*REAL(JL) 
! attention: commenter/decommenter le END DO plus loin
! STERAO VER_USER
    DO JK =IKB,IKE
      DO JJ = IJB,IJE
        DO JI = IIB,IIE
