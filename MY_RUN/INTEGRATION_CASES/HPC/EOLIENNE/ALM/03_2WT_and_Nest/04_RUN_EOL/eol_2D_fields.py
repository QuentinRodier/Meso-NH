...
Xpos2 = farm_data.values[1,0] # Quickly add 2nd WT
# Hub height
...
#
# Variables to print
THRUT   = nc.Dataset(nc_file).variables['THRUT'][0,:]
THRUMME = nc.Dataset(nc_file).variables['THRUMME'][0,:]
TORQT   = nc.Dataset(nc_file).variables['TORQT'][0,:]
TORQMME = nc.Dataset(nc_file).variables['TORQMME'][0,:]
POWT    = nc.Dataset(nc_file).variables['POWT'][0,:]
POWMME  = nc.Dataset(nc_file).variables['POWMME'][0,:]
...
# Quickly add 2nd WT
ax_cut1.plot([Xpos2,Xpos2], [Hhub-R,Hhub+R], 'black', lw=2)
ax_cut2.plot([Xpos2,Xpos2], [Ypos-R,Ypos+R], 'black', lw=2)
#
#	 e. Cut planes
#
...
textstr1 = '\n'.join((
    'Wind turbine n.1 :',
    r'$F_T = %.2f$ kN' % (THRUT[0]/1000, ),
    r'$\overline{F_T} =%.2f$ kN' % (THRUMME[0]/1000, ),
    r'$T =%.2f$ MNm' % (TORQT[0]/1000000, ),
    r'$\overline{T} = %.2f$ MNm' % (TORQMME[0]/1000000, ),
    r'$P = %.2f$ MW' % (POWT[0]/1000000, ),
    r'$\overline{P} = %.2f$ MW' % (POWMME[0]/1000000, )))

textstr2 = '\n'.join((
    'Wind turbine n.2 :',
    r'$F_T = %.2f$ kN' % (THRUT[1]/1000, ),
    r'$\overline{F_T} =%.2f$ kN' % (THRUMME[1]/1000, ),
    r'$T =%.2f$ MNm' % (TORQT[1]/1000000, ),
    r'$\overline{T} = %.2f$ MNm' % (TORQMME[1]/1000000, ),
    r'$P = %.2f$ MW' % (POWT[1]/1000000, ),
    r'$\overline{P} = %.2f$ MW' % (POWMME[1]/1000000, )))

props = dict(boxstyle='round', facecolor='wheat', alpha=0.5)
ax_cut3.text(-0.25, -0.8, textstr1,transform=ax_cut3.transAxes, fontsize=14,
        verticalalignment='top', bbox=props)
ax_cut3.text(0.85, -0.8, textstr2,transform=ax_cut3.transAxes, fontsize=14,
        verticalalignment='top', bbox=props)
#
#
#plt.show()
