...
USE MODD_PARAMETERS
USE MODD_GRID_n
USE MODD_BLANK_n, ONLY : XDUMMY1,    & !rate of cooling (K/s)
                       XDUMMY2,    & !vertical size of the disturbance
                       XDUMMY3,    & !horizontal size of the disturbance
                       XDUMMY4,    & !relative begining of the disturbance
...
!!!!  SQUALL LINE COPT81
!
!*      3.  DEFINITION OF THE THERMAL PERTURBATION
!       ------------------------------------------
!
IF (CCONF=='START'.AND. (KTCOUNT*PTSTEP)<=(2.*XDUMMY5)) THEN
!
! SIZE OF THE COLD POOL
!
  JIBEG=XDUMMY4*SIZE(XDXHAT)
  JIEND=JIBEG+NINT(XDUMMY3/XDXHAT(JIBEG))
!
  DO JK =1,SIZE(XZHAT)
    IF (XZHAT(JK)<=XDUMMY2) THEN
      DO JI = JIBEG,JIEND
        PRTHS(JI,1+JPHEXT,JK) = PRTHS(JI,1+JPHEXT,JK)-PRHODJ(JI,1+JPHEXT,JK)*XDUMMY1
      END DO
    END IF
  END DO
!
