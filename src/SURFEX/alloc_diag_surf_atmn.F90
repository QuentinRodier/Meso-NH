...
ALLOCATE(D%XSFCO2NAT(KSIZE_FULL))
ALLOCATE(D%XSFCO2ANT(KSIZE_FULL))
!
...
ALLOCATE(D%XTSLSI (KSIZE_FULL))
ALLOCATE(D%XEMIS  (KSIZE_FULL))
!
ALLOCATE(D%XTA_FRC  (KSIZE_FULL))
ALLOCATE(D%XQA_FRC  (KSIZE_FULL))
ALLOCATE(D%XU_FRC   (KSIZE_FULL))
ALLOCATE(D%XV_FRC   (KSIZE_FULL))
ALLOCATE(D%XCO2_FRC (KSIZE_FULL))
ALLOCATE(D%XRAIN_FRC(KSIZE_FULL))
ALLOCATE(D%XSNOW_FRC(KSIZE_FULL))
!
D%XSFCO2   (:) = XUNDEF
D%XSFCO2NAT(:) = XUNDEF
D%XSFCO2ANT(:) = XUNDEF
...
IF (.NOT.OCUM) THEN
  ...
  ALLOCATE(DA%XWIFF_MEAN  (KSIZE1))
  ALLOCATE(DA%XWIDD_MEAN   (KSIZE1))
  !
  ALLOCATE(DA%XSWBD   (KSIZE1,KSIZE2))
  ALLOCATE(DA%XSWBU   (KSIZE1,KSIZE2))
  ALLOCATE(DA%XALB_DIR(KSIZE1,KSIZE2))
  ALLOCATE(DA%XALB_SCA(KSIZE1,KSIZE2))
  !
  ...
  !  
  ALLOCATE(DA%XSWD_SCA(KSIZE1))
  !  
ELSE
  !
  ALLOCATE(DA%XSWBD   (0,0))
  ALLOCATE(DA%XSWBU   (0,0))
  ALLOCATE(DA%XALB_DIR(0,0))
  ALLOCATE(DA%XALB_SCA(0,0))  
  !
  ALLOCATE(DA%XSWD_SCA(0))
  !
ENDIF
!
