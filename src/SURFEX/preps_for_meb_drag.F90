...
#else
     WRITE( CMNHMSG(1), '( "MAXVAL(PH_VEG)=", E15.8 )' ) MAXVAL(PH_VEG)
     WRITE( CMNHMSG(2), '( "MAXVAL(PDISPH)=", E15.8 )' ) MAXVAL(PDISPH)
     WRITE( CMNHMSG(3), '( "MINVAL(PZREF) =", E15.8 )' ) MINVAL(PZREF)
     WRITE( CMNHMSG(4), '( "MINVAL(PUREF) =", E15.8 )' ) MINVAL(PUREF)
     WRITE( CMNHMSG(5), '( "Forcing height for wind or temperature too low" )' )
     CALL PRINT_MSG( NVERB_FATAL, 'GEN', 'PREPS_FOR_MEB_DRAG' )
#endif
   ENDIF
!
ELSE
