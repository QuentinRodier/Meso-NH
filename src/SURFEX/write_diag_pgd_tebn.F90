...
DO JLAYER=1,TOP%NTEB_SOIL
  ...
  ELSE
    WRITE(YRECFM,FMT='(A,I2.1)') 'HC_ROAD',JLAYER
  END IF
...
DO JLAYER=1,TOP%NTEB_SOIL
  ...
  ELSE
    WRITE(YRECFM,FMT='(A,I2.1)') 'TC_ROAD',JLAYER
  END IF
...
IF (TOP%CBEM=='BEM') THEN
   !
   YRECFM='RESIDENTIAL'
   YCOMMENT='Residential fraction'
   CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,B%XRESIDENTIAL(:),IRESP,HCOMMENT=YCOMMENT)
   !
   DO JLAYER=1,BOP%NBEMCOMP
      WRITE(YRECFM,FMT='(A8,I1.1)') 'FRACOMP_',JLAYER
      YCOMMENT='BEM compartment frequency'
      CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,B%XFRACOMP(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
   ENDDO
   !
   YRECFM='N_FLOOR'
   ...
   DO JLAYER=1,BOP%NMASS_LAYER
     WRITE(YRECFM,FMT='(A,I1.1)') 'HC_MASS',JLAYER
     YCOMMENT='MASS Heat Capacity'
     CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,B%XHC_MASS(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
   END DO
   !
   DO JLAYER=1,BOP%NMASS_LAYER
     WRITE(YRECFM,FMT='(A,I1.1)') 'TC_MASS',JLAYER
     YCOMMENT='MASS thermal conductivity'
     CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,B%XTC_MASS(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
   END DO
   !
   DO JLAYER=1,BOP%NMASS_LAYER
