...
!     #########
      SUBROUTINE ISBA_SNOW_AGR(KK, PK, PEK, DMK, DK, DEK, AT,              &
                               OMEB, OMEB_LITTER, PEXNS, PEXNA, PTA, PQA,  &
                               PZREF, PUREF, PDIRCOSZW, PVMOD, PRR, PSR,   &
                               PEMIS, PALB, PUSTAR, PLES3L, PLEL3L,        &
                               PEVAP3L, PQS3L, PALB3L, PGRNDFLUX,          &
                               PFLSN_COR, PEMIST, PPALPHAN, PSNOWSFCH      )
!     ##########################################################################
!
!
...
!!      A. Boone    11/2014  MEB
!!      B. Decharme 04/2019  Budget diags
!-------------------------------------------------------------------------------
!
!*       0.     DECLARATIONS
!               ------------
...
REAL, DIMENSION(:), INTENT(IN)    :: PSNOWSFCH  ! snow surface layer pseudo-heating term
!
REAL, DIMENSION(:), INTENT(IN)    :: PGRNDFLUX  ! snow/soil-biomass interface flux (W/m2)
...
!
  DEK%XUSTAR_SN_FR(:) = PUSTAR (:) ! NOTE for now, this is same as total Ustar (includes snow)       
!
  DEK%XLER_SN_FR(:) = DEK%XLEV_CV(:) - DEK%XLETR_CV(:) ! LER does not include intercepted snow sublimation
!
  DEK%XLEI_SN_FR(:) = DEK%XLEGI(:) + DEK%XLEI_FLOOD(:) + DEK%XLES(:) + DEK%XLES_CV(:)
!

  DEK%XLE_SN_FR(:) = DEK%XLEG_SN_FR(:) + DEK%XLEGI_SN_FR(:) + DEK%XLEV_SN_FR(:) &
                   + DEK%XLES_CV(:) + DEK%XLE_FLOOD(:) + DEK%XLEI_FLOOD(:) ! LE includes intercepted snow sublimation
!
...
!
  DK%XGFLUX(:) = DK%XRN(:) - DK%XH(:) - DK%XLE(:) + DMK%XHPSNOW(:) 
!
! Aerodynamic resistance of the gird-cell
!
  DK%XARES(:)  = PEK%XRESA(:)
!
ELSE
...
!
      DK%XTS(:) = (1.0-PEK%XPSN(:))*PEK%XTG(:,1)+PEK%XPSN(:)*DMK%XSNOWTEMP(:,1)
!
!     Effective surface radiating temperature:
!
      DK%XALBT (:) = PALB (:)*(1.0-PEK%XPSN(:)) + PEK%XPSN(:)*PALB3L(:)
      PEMIST   (:) = PEMIS(:)*(1.0-PEK%XPSN(:)) + PEK%XPSN(:)*PEK%TSNOW%EMIS(:)
!
!     Effective surface radiating temperature: X**n = EXP(n*LOG(X))
! 
      ZWORK(:) = (1.0-PEK%XPSN(:))*PEMIS(:)*PEK%XTG(:,1)**4 + PEK%XPSN(:)*PEK%TSNOW%EMIS(:)*DMK%XSNOWTEMP(:,1)**4                           
      DK%XTSRAD(:) = EXP(0.25*LOG(ZWORK(:)/PEMIST(:)))
!
...
!
      DK%XRN   (:) = (1.0-PEK%XPSN(:)) * DK%XRN(:) + PEK%XPSN(:) * DMK%XRNSNOW(:)
      DK%XH    (:) = (1.0-PEK%XPSN(:)) * DK%XH (:) + PEK%XPSN(:) * DMK%XHSNOW(:)
!  
...
!
      DEK%XLE_FLOOD (:) = KK%XFF(:) * DEK%XLE_FLOOD(:)
      DEK%XLEI_FLOOD(:) = KK%XFF(:) * DEK%XLEI_FLOOD(:)
!
!     Total evapotranspiration flux (kg/m2/s):
!
      DK%XEVAP(:) = (DEK%XLEV(:)+DEK%XLEG(:)+DEK%XLE_FLOOD(:))/PK%XLVTT(:) &
                  + (DEK%XLEGI(:)+DEK%XLEI_FLOOD(:))/PK%XLSTT(:)          &
                  + PEK%XPSN(:) * PEVAP3L(:)
!
!     Total potential evapotranspiration flux (kg/m2/s):
!
      DEK%XEPOT(:)     = (1.-PEK%XPSN(:)) * DEK%XEPOT(:) + PEK%XPSN(:) * PEVAP3L(:)
!
!     ISBA-ES/SNOW3L delta energy:
!
      DEK%XDELHEATN_SFC (:) = PEK%XPSN(:) * (DEK%XDELHEATN_SFC(:)-PSNOWSFCH(:)+DEK%XSWNET_N(:)-DEK%XSWNET_NS(:))
      DEK%XDELHEATN     (:) = PEK%XPSN(:) * DEK%XDELHEATN     (:)
      DEK%XDELPHASEN    (:) = PEK%XPSN(:) * DEK%XDELPHASEN    (:)
      DEK%XDELPHASEN_SFC(:) = PEK%XPSN(:) * DEK%XDELPHASEN_SFC(:)
!
...
!
      ...
      DEK%XRESTOREN (:) = PEK%XPSN(:) * DEK%XRESTOREN (:)
...
!
      DEK%XGRNDFLUX(:) = PEK%XPSN(:) * (PGRNDFLUX(:)+PFLSN_COR(:))
      DEK%XMELTADV (:) = PEK%XPSN(:) * DEK%XMELTADV(:)
!
!     Total evaporative flux (W/m2) :
!
      DK%XLE(:) = DEK%XLEG(:) + DEK%XLEV(:) + DEK%XLES(:) + DEK%XLESL(:) &
...
!*      0.2    declarations of local variables
!
REAL, DIMENSION(SIZE(PTA)) :: ZFP, ZRRCOR, ZVMOD, ZAC
!
REAL(KIND=JPRB) :: ZHOOK_HANDLE
!
!-------------------------------------------------------------------------------
...
!
   CALL SURFACE_CDCH_1DARP(PZREF, DK%XZ0EFF, DK%XZ0H, ZVMOD, PTA, DK%XTS,   &
                           PQA, DK%XQS, AT, DK%XCD, DK%XCDN, DK%XCH, DK%XRI )
!
ELSE
!
   CALL SURFACE_AERO_COND(DK%XRI, PZREF, PUREF, ZVMOD, DK%XZ0, DK%XZ0H, ZAC, DK%XARES, DK%XCH)
