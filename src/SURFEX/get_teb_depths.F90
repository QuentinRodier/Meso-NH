...
!     #########
      SUBROUTINE GET_TEB_DEPTHS (DTCO,  &
                                 ...
                                 PD_ROOF, PD_ROAD, PD_WALL, PD_FLOOR, PD_MASS,HDIR)
!     ##############################################################
!
!!**** *CONVERT_COVER* 
!!
...
USE MODD_DATA_COVER_PAR, ONLY : NCOVER, NTYPE, NDATA_ROOF_LAYER, NDATA_ROAD_LAYER, &
                                NDATA_WALL_LAYER, NDATA_FLOOR_LAYER, NDATA_MASS_LAYER
!
...
REAL                                 :: ZD_ROAD! for reading of new versions of TEB
!
...
INTEGER :: IDATA1                   ! number of covers in ecoclimap
INTEGER :: ILU                      ! number of points
LOGICAL           :: GECOCLIMAP, GECOSG
LOGICAL           :: GDATA, GDIM, GOLD_NAME          ! T if depth is to be read in the file
LOGICAL           :: GREAD_EXT
...
ALLOCATE(YDTCO%XDATA_GARDEN(IDATA1))
ALLOCATE(YDTCO%XDATA_TOWN  (IDATA1))
ALLOCATE(YDTCO%XDATA_BLD   (IDATA1))
!
IF (IDATA1/=SIZE(XDATA_D_ROOF,1)) THEN
...
ENDIF
!
GOLD_NAME=(IVERSION_PREP<8 .OR. (IVERSION_PREP==8 .AND. IBUGFIX_PREP<2))
IF (PRESENT(PD_ROOF)) THEN
  ...
  YAREA   = 'BLD'
  ALLOCATE(ZDATA(IDATA1,IDATA_LAYER))
  IF (GECOSG) THEN
    ZDATA = XDATA_D_ROOF
  ELSE
    CALL DEFAULT_DATA_COVER(PDATA_TOWN=YDTCO%XDATA_TOWN,PDATA_D_ROOF=ZDATA)
...
IF (PRESENT(PD_MASS)) THEN
  ...
  IF (GOLD_NAME) THEN
     ...
     YRECFM3 = 'D_FLOOR'
  ELSE
     YRECFM0 = 'PAR_MA_LAYER'
     YRECFM1 = 'L_D_MASS'
     YRECFM2 = 'D_D_MASS'
     YRECFM3 = 'D_MASS'
  ENDIF
  IDATA_LAYER = NDATA_MASS_LAYER
  ILU     = SIZE(PD_MASS,1)
  ILAYER  = SIZE(PD_MASS,2)
  YAREA   = 'BLD'
  ALLOCATE(ZDATA(IDATA1,IDATA_LAYER))
  IF (GECOSG) THEN
    ZDATA = XDATA_D_MASS
...
IF ( (IVERSION_PGD>8 .OR. (IVERSION_PGD==8 .AND. IBUGFIX_PGD>=2) ) &
    .AND. PRESENT(PD_ROAD) ) THEN
  CALL OPEN_AUX_IO_SURF(HFILEPGD,HFILEPGDTYPE,'TOWN  ')
  ALLOCATE(ZD(ILU,ILAYER)) 
  DO JL=1,SIZE(PD_ROAD,2)
    WRITE(YLVL,'(I2)') JL
    YRECFM='TEB_SGRID'//ADJUSTL(YLVL(:LEN_TRIM(YLVL)))
    CALL READ_SURF(HFILEPGDTYPE,YRECFM,ZD_ROAD,IRESP)
    ZD(:,JL) = ZD_ROAD
  END DO
  GDATA = .TRUE. 
  CALL CLOSE_AUX_IO_SURF(HFILEPGD,HFILEPGDTYPE) 
ELSE IF (IVERSION_PGD<7 .OR. (IVERSION_PGD==7 .AND. IBUGFIX_PGD<=2)) THEN
  GDATA = .FALSE.
ELSE
  !
  CALL OPEN_AUX_IO_SURF(HFILEPGD,HFILEPGDTYPE,'TOWN  ')
