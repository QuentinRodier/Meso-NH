...
!----------------------------------------------------------------------------
!
IF (LHOOK) CALL DR_HOOK('GET_MESH_INDEX_IGN',0,ZHOOK_HANDLE)
...
!
!       write (*,*),'APPEL  XY_IGN'
!       print*,'LON LAT ENTREE : ',plon(1),plat(1)
...
!
DO JI=1,SIZE(PLON)
  IF (     ZX(JI)<XX_MIN .OR. ZX(JI)>XX_MAX        &
         .OR. ZY(JI)<XY_MIN .OR. ZY(JI)>XY_MAX ) THEN   
    KINDEX(JI) = 0
    IF (KSSO/=0) THEN
      KISSOX(JI) = 0
      KISSOY(JI) = 0
...
! 
  IF (IVAR.NE.0) THEN
    ...
    !le point qu'on regarde est-il dans un point de grille du domaine          
    DO JL=IVAR,SIZE(XXLIM)
      IF( ZX(JI)> XXLIM(JL) .AND. ZX(JI) < XXLIM(JL)+XDX(JL)   & 
        .AND. ZY(JI) > XYLIM(JL) .AND. ZY(JI) < XYLIM(JL)+XDY(JL) ) THEN
        
        KINDEX(JI) = JL
        IVAR = JL+1

!   write(*,'(A,2F8.4,2F11.2,A,I4,A1)'),            &
!   'point ', PLAT(JI), PLON(JI),ZX(JI),ZY(JI),' index : ',KINDEX (JI),'='
!
!
...
!
        IF (KSSO/=0) THEN
          KISSOX(JI) = 1 + INT( FLOAT(KSSO) *             &
                           (ZX(JI)-XXLIM(KINDEX(JI)))/XDX(KINDEX(JI)) )   
          KISSOY(JI) = 1 + INT( FLOAT(KSSO) *             &
                           (ZY(JI)-XYLIM(KINDEX(JI)))/XDY(KINDEX(JI)) )   
        END IF
        EXIT
      ENDIF
    ENDDO
...
DEALLOCATE(ZX )
DEALLOCATE(ZY )
IF (LHOOK) CALL DR_HOOK('GET_MESH_INDEX_IGN',1,ZHOOK_HANDLE)
!-------------------------------------------------------------------------------
!
