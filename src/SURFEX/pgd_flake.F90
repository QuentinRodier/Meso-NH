...
ELSE
  ...
  ENDIF
  !
  IF(XUNIF_WATER_DEPTH==XUNDEF)XUNIF_WATER_DEPTH=10.0
  !
  CATYPE='INV'
  CALL PGD_FIELD(DTCO, UG, U, USS, &
                 HPROGRAM,'water depth','WAT',YWATER_DEPTH,YWATER_DEPTHFILETYPE,XUNIF_WATER_DEPTH,F%XWATER_DEPTH(:))
  !
  IF(MINVAL(F%XWATER_DEPTH(:))<0.0)THEN
     WRITE(ILUOUT,*)'Flake depth < 0 :',MINVAL(F%XWATER_DEPTH(:))
     WRITE(ILUOUT,*)'Input file must be wrong !'
     CALL ABOR1_SFX('PGD_FLAKE: FLAKE DEPTH < 0 ; WRONG INPUT FILE')
  ENDIF
  !
ENDIF
!
