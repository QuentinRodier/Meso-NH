...
USE MODD_XIOS,           ONLY : LALLOW_ADD_DIM, YTG_LAYER_DIM_NAME,      &
                                YGROUND_LAYER_DIM_NAME, YPATCH_DIM_NAME, &
                                YSWBAND_DIM_NAME
...
!
#ifdef SFX_OL
IF (ID%O%LSURF_BUDGET .AND. DUO%LRESETCUMUL .AND. ID%O%LSURF_BUDGETC .AND. .NOT.LRESET_DIAG_ol) THEN
  !
...
IF (ID%O%LSURF_BUDGETC) THEN
  ...
  !
  YRECFM='EVAPC_ISBA'
  YCOMMENT='total evaporative flux for tile nature'//' (kg/m2)'
...
IF(NPE%AL(1)%TSNOW%SCHEME=='3-L' .OR. NPE%AL(1)%TSNOW%SCHEME=='CRO')THEN  
  ...
  YCOMMENT='blowing snow sublimation for tile nature'//YUNITW
...
YCOMMENT='drainage for tile nature'//YUNITW
...
YCOMMENT='runoff for tile nature'//YUNITW
...
IF(IO%CHORT=='SGH'.OR.IO%CISBA=='DIF')THEN
  ...
  YCOMMENT='horton runoff for tile nature'//YUNITW
...
IF(ID%DE%LWATER_BUDGET .OR. OFLAG)THEN 
  ...
  YCOMMENT='isba water budget as residue'//YUNITW
...
IF(ID%DE%LWATER_BUDGET .OR. OFLAG)THEN 
  !
  YRECFM='DWG'//TRIM(HTERM)
  YCOMMENT='change in liquid soil moisture per patch'//YUNITW
    DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
  ...
  !
  YRECFM='DWGI'//TRIM(HTERM)
  YCOMMENT='change in solid soil moisture per patch'//YUNITW
    DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
  ...
  !
  YRECFM='DWR'//TRIM(HTERM)
  YCOMMENT='change in water on canopy per patch'//YUNITW
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
  ...
  !
  YRECFM='DSWE'//TRIM(HTERM)
  YCOMMENT='change in snow water equivalent per patch'//YUNITW
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
  ...
  !
  YRECFM='WATBUD'//TRIM(HTERM)
  YCOMMENT='isba water budget as residue per patch'//YUNITW
  DO JP=1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(DUO%CSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
