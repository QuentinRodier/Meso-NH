...
!     #########
    SUBROUTINE VEGETATION_EVOL(IO, DTI, PK, PEK, OAGRIP, PTSTEP, KMONTH, KDAY, PTIME,    &
                               PLAT, PRHOA, P_CO2, ISSK, PRESP_BIOMASS_INST, LBIOM_REAP, &
                               PSWDIR, ONLY_MULTIS, OHVEG_ONLY)  
!   ###############################################################
!!****  *VEGETATION EVOL*
!!
!!    PURPOSE
...
!
LOGICAL,              INTENT(IN),   OPTIONAL :: ONLY_MULTIS ! To only done the multi-irrigation computation
LOGICAL,                            OPTIONAL :: OHVEG_ONLY
!
...
REAL :: ZDILUDEC
!
!* flag for high vegetation computations only
!
LOGICAL GHVEG_ONLY
!
...
!
IF ( .NOT.PRESENT(ONLY_MULTIS) ) THEN
!
INI=SIZE(PEK%XTG,1)
INL=SIZE(PEK%XTG,2)
!
GHVEG_ONLY=.FALSE.
IF (PRESENT(OHVEG_ONLY)) GHVEG_ONLY = OHVEG_ONLY
!
ZLOG2 = LOG(2.0)
!
ZTG_SOIL(:) = 0.0
...
ENDDO
ENDIF ! FOR IF (.NOT.PRESENT(ONLY_MULTIS)) THEN
!
...
!              -------------------------------------------
!
IF (GMASK .AND. .NOT.PRESENT(ONLY_MULTIS) ) THEN
  !
  ...
  ENDIF
  IF(.NOT.DTI%LIMP_VEG .AND. .NOT. GHVEG_ONLY) THEN
    WHERE( PEK%XVEG(:) > 0. ) &
