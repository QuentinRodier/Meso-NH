...
!     #########
      SUBROUTINE WRITESURF_PGD_ISBA_PAR_n (HSELECT, OECOSG, DTV, HPROGRAM)
!     ################################################
!
...
USE MODI_WRITE_SURF
!
USE MODD_AGRI ,ONLY : LAGRIP, LMULTI_SEASON
!
...
!
YRECFM='L_VEG_IRRIG'
YCOMMENT='NVEGTYPE IRRIGATED'
IF ( .NOT. ANY(DTV%NPAR_VEG_IRR_USE==0) ) THEN
  ALLOCATE(LWORK(DTV%NVEGTYPE))
  LWORK = .FALSE.
  !
  DO JV=1,DTV%NVEGTYPE
    IF ( ANY(DTV%NPAR_VEG_IRR_USE==JV) ) LWORK(JV) = .TRUE.
    !                                    IWORK(JV) = MAXLOC(DTV%NPAR_VEG_IRR_USE, 1, MASK = DTV%NPAR_VEG_IRR_USE==JV)
  ENDDO
  !
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,LWORK,IRESP,HCOMMENT=YCOMMENT,HDIR='-')
  DEALLOCATE(LWORK)
ENDIF
!
YRECFM='L_IRRIGTYPE'
YCOMMENT=YRECFM
 CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,DTV%LDATA_IRRIGTYPE,IRESP,HCOMMENT=YCOMMENT,HDIR='-')
YCOMMENT='X_Y_IRRIGTYPE' 
DO JV=1,DTV%NVEGTYPE
    IF (DTV%LDATA_IRRIGTYPE(JV)) THEN
      WRITE(YRECFM,FMT='(A10,I2.2)') 'D_IRRTYP_V',JV       
      CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,DTV%XPAR_IRRIGTYPE(:,JV),IRESP,HCOMMENT=YCOMMENT)
    ENDIF
ENDDO
!
IF (ANY(DTV%LDATA_IRRIGTYPE)) THEN
  !
  YRECFM='L_WATSUP'
  YCOMMENT=YRECFM
  ...
  IF ( OECOSG ) THEN
    YRECFM='L_IRRIGFRAC'
    YCOMMENT=YRECFM
      CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,DTV%LDATA_IRRIGFRAC,IRESP,HCOMMENT=YCOMMENT,HDIR='-')
    YCOMMENT='X_Y_IRRIGFRAC'
    DO JV=1,DTV%NVEGTYPE
      IF (DTV%LDATA_IRRIGFRAC(JV)) THEN
        WRITE(YRECFM,FMT='(A10,I2.2)') 'D_IRRFRA_V',JV
        CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,DTV%XPAR_IRRIGFRAC(:,JV),IRESP,HCOMMENT=YCOMMENT)
      ENDIF
    ENDDO
  ENDIF
  !
  YRECFM='L_IRRIGFREQ'
  YCOMMENT=YRECFM
   CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,DTV%LDATA_IRRIGFREQ,IRESP,HCOMMENT=YCOMMENT,HDIR='-')
  YCOMMENT='X_Y_IRRIGFREQ' 
  DO JV=1,DTV%NVEGTYPE
    IF (DTV%LDATA_IRRIGFREQ(JV)) THEN
      WRITE(YRECFM,FMT='(A10,I2.2)') 'D_IRRFRQ_V',JV       
      CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,DTV%XPAR_IRRIGFREQ(:,JV),IRESP,HCOMMENT=YCOMMENT)
    ENDIF
  ENDDO
  !
  YRECFM='L_IRRIGTIME'
  YCOMMENT=YRECFM
   CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,DTV%LDATA_IRRIGTIME,IRESP,HCOMMENT=YCOMMENT,HDIR='-')
  YCOMMENT='X_Y_IRRIGTIME' 
  DO JV=1,DTV%NVEGTYPE
    IF (DTV%LDATA_IRRIGTIME(JV)) THEN
      WRITE(YRECFM,FMT='(A10,I2.2)') 'D_IRRTIM_V',JV       
      CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,DTV%XPAR_IRRIGTIME(:,JV),IRESP,HCOMMENT=YCOMMENT)
    ENDIF
  ENDDO
  !
  YRECFM='L_F2THRESHOLD'
  YCOMMENT=YRECFM
   CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,DTV%LDATA_F2THRESHOLD,IRESP,HCOMMENT=YCOMMENT,HDIR='-')
  YCOMMENT='X_Y_F2THRESHOLD'   
  DO JTIME=1,DTV%NTIME
    DO JV=1,DTV%NVEGTYPE
      IF (DTV%LDATA_F2THRESHOLD((JTIME-1)*DTV%NVEGTYPE+JV)) THEN
        WRITE(YRECFM,FMT='(A7,I2.2,A1,I2.2)') 'D_F2T_T',JTIME,'V',JV  
        CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,DTV%XPAR_F2THRESHOLD(:,JTIME,JV),IRESP,HCOMMENT=YCOMMENT)
      ENDIF
    ENDDO
  ENDDO
  !
ENDIF
!
IF ( ANY(DTV%LDATA_IRRIGTYPE) .OR. LAGRIP ) THEN
  YRECFM='L_SEED_M'
  YCOMMENT=YRECFM
  ...
  IF (ANY(DTV%LDATA_SEED_M)) THEN
    ...
    YRECFM='L_SEED_S2_M'
    YCOMMENT=YRECFM
     CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,DTV%LDATA_SEED_S2_M,IRESP,HCOMMENT=YCOMMENT,HDIR='-')
    YCOMMENT='X_Y_SEED_S2_M'
    DO JV=1,DTV%NVEGTYPE
      IF (DTV%LDATA_SEED_S2_M(JV)) THEN
        WRITE(YRECFM,FMT='(A10,I2.2,A1)') 'D_SEED2M_V',JV
        CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,DTV%XPAR_SEED_S2_M(:,JV),IRESP,HCOMMENT=YCOMMENT)
      ENDIF
    ENDDO
    !
    YRECFM='L_SEED_S2_D'
    YCOMMENT=YRECFM
     CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,DTV%LDATA_SEED_S2_D,IRESP,HCOMMENT=YCOMMENT,HDIR='-')
    YCOMMENT='X_Y_SEED_S2_D'
    DO JV=1,DTV%NVEGTYPE
      IF (DTV%LDATA_SEED_S2_D(JV)) THEN
        WRITE(YRECFM,FMT='(A10,I2.2)') 'D_SEED2D_V',JV
        CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,DTV%XPAR_SEED_S2_D(:,JV),IRESP,HCOMMENT=YCOMMENT)
      ENDIF
    ENDDO
    !
    YRECFM='L_REAP_S2_M'
    YCOMMENT=YRECFM
     CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,DTV%LDATA_REAP_S2_M,IRESP,HCOMMENT=YCOMMENT,HDIR='-')
    YCOMMENT='X_Y_REAP_S2_M'
    DO JV=1,DTV%NVEGTYPE
      IF (DTV%LDATA_REAP_S2_M(JV)) THEN
        WRITE(YRECFM,FMT='(A10,I2.2)') 'D_REAP2M_V',JV
        CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,DTV%XPAR_REAP_S2_M(:,JV),IRESP,HCOMMENT=YCOMMENT)
      ENDIF
    ENDDO
    !
    YRECFM='L_REAP_S2_D'
    YCOMMENT=YRECFM
     CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,DTV%LDATA_REAP_S2_D,IRESP,HCOMMENT=YCOMMENT,HDIR='-')
    YCOMMENT='X_Y_REAP_S2_D'
    DO JV=1,DTV%NVEGTYPE
      IF (DTV%LDATA_REAP_S2_D(JV)) THEN
        WRITE(YRECFM,FMT='(A10,I2.2)') 'D_REAP2D_V',JV
        CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,DTV%XPAR_REAP_S2_D(:,JV),IRESP,HCOMMENT=YCOMMENT)
      ENDIF
    ENDDO
    !
    IF ( LMULTI_SEASON ) THEN
      ...
      YRECFM='L_SEED_S3_M'
      YCOMMENT=YRECFM
       CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,DTV%LDATA_SEED_S3_M,IRESP,HCOMMENT=YCOMMENT,HDIR='-')
      YCOMMENT='X_Y_SEED_S3_M'
      DO JV=1,DTV%NVEGTYPE
        IF (DTV%LDATA_SEED_S3_M(JV)) THEN
          WRITE(YRECFM,FMT='(A10,I2.2,A1)') 'D_SEED3M_V',JV
          CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,DTV%XPAR_SEED_S3_M(:,JV),IRESP,HCOMMENT=YCOMMENT)
        ENDIF
      ENDDO
      !
      YRECFM='L_SEED_S3_D'
      YCOMMENT=YRECFM
       CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,DTV%LDATA_SEED_S3_D,IRESP,HCOMMENT=YCOMMENT,HDIR='-')
      YCOMMENT='X_Y_SEED_S3_D'
      DO JV=1,DTV%NVEGTYPE
        IF (DTV%LDATA_SEED_S3_D(JV)) THEN
          WRITE(YRECFM,FMT='(A10,I2.2)') 'D_SEED3D_V',JV
          CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,DTV%XPAR_SEED_S3_D(:,JV),IRESP,HCOMMENT=YCOMMENT)
        ENDIF
      ENDDO
      !
      YRECFM='L_REAP_S3_M'
      YCOMMENT=YRECFM
       CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,DTV%LDATA_REAP_S3_M,IRESP,HCOMMENT=YCOMMENT,HDIR='-')
      YCOMMENT='X_Y_REAP_S3_M'
      DO JV=1,DTV%NVEGTYPE
        IF (DTV%LDATA_REAP_S3_M(JV)) THEN
          WRITE(YRECFM,FMT='(A10,I2.2)') 'D_REAP3M_V',JV
          CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,DTV%XPAR_REAP_S3_M(:,JV),IRESP,HCOMMENT=YCOMMENT)
        ENDIF
      ENDDO
      !
      YRECFM='L_REAP_S3_D'
      YCOMMENT=YRECFM
       CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,DTV%LDATA_REAP_S3_D,IRESP,HCOMMENT=YCOMMENT,HDIR='-')
      YCOMMENT='X_Y_REAP_S3_D'
      DO JV=1,DTV%NVEGTYPE
        IF (DTV%LDATA_REAP_S3_D(JV)) THEN
          WRITE(YRECFM,FMT='(A10,I2.2)') 'D_REAP3D_V',JV
          CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,DTV%XPAR_REAP_S3_D(:,JV),IRESP,HCOMMENT=YCOMMENT)
        ENDIF
      ENDDO
      !  
    ENDIF
