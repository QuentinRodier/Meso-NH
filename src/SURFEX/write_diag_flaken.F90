...
USE MODI_WRITE_DIAG_MIP_FLAKE_n
!
...
!
!
ITIME_RATIO=NINT(FM%F%TTIME%TIME/FM%DFO%XDIAG_TSTEP)
GWRITE     =(FM%DFO%XDIAG_TSTEP==XUNDEF.OR.ABS(ITIME_RATIO*FM%DFO%XDIAG_TSTEP-FM%F%TTIME%TIME)<1.E-3)
!
! * Write Diag other than pgd
!
IF (GWRITE.AND.HWRITE/='PGD'.AND.FM%DFO%LDIAG_MIP) THEN
!
! * Write cmip Diag
!
    CALL WRITE_DIAG_MIP_FLAKE_n(DTCO, DUO, U, FM%F, FM%DF, HPROGRAM)
!
