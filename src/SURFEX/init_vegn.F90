...
IF ((IO%CPHOTO=='NIT' .OR. IO%CPHOTO=='NCB') .AND. PK%NSIZE_P > 0 ) THEN
  DO JI=1,PK%NSIZE_P    
    IF(PEK%XLAI(JI)/=XUNDEF) THEN
      ...
      IF (.NOT.DTV%LIMP_Z0)   &
         PEK%XZ0  (JI) = Z0V_FROM_LAI(PEK%XLAI(JI),PK%XH_TREE(JI),PK%XVEGTYPE_PATCH(JI,:),NPAR_VEG_IRR_USE=DTV%NPAR_VEG_IRR_USE)
      IF (.NOT.DTV%LIMP_VEG)  &
        PEK%XVEG (JI) = VEG_FROM_LAI(PEK%XLAI(JI),PK%XVEGTYPE_PATCH(JI,:),NPAR_VEG_IRR_USE=DTV%NPAR_VEG_IRR_USE)
      IF (.NOT.DTV%LIMP_EMIS) &
        PEK%XEMIS(JI) = EMIS_FROM_VEG(PEK%XVEG(JI),PK%XVEGTYPE_PATCH(JI,:),NPAR_VEG_IRR_USE=DTV%NPAR_VEG_IRR_USE)
    END IF
    IF(.NOT.DTV%LIMP_H_VEG.AND.PEK%XLAI(JI)/=XUNDEF) THEN
       PEK%XH_VEG(JI) = H_VEG_FROM_LAI(PEK%XLAI(JI),PK%XH_TREE(JI),PK%XVEGTYPE_PATCH(JI,:),NPAR_VEG_IRR_USE=DTV%NPAR_VEG_IRR_USE)
    END IF        
  END DO
END IF
!
