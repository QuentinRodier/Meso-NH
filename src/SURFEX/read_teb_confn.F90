...
!     #########
      SUBROUTINE READ_TEB_CONF_n (CHT, DMTO, DGO, DUT, T, TOP, TPN, HPROGRAM)
!     #######################################################
!
...
IF (LNAM_READ) THEN
 ...
 CALL POSNAM(INAM,'NAM_TEB_PANELN',GFOUND,ILUOUT)
 IF (GFOUND) READ(UNIT=INAM,NML=NAM_TEB_PANELn)
 !
 CALL TEST_NAM_VAR_SURF(ILUOUT,'CZ0H',CZ0H,'MASC95','BRUT82','KAND07')
 CALL TEST_NAM_VAR_SURF(ILUOUT,'CZ0EFF_GD',CZ0EFF_GD,'LR21','NONE')
 CALL TEST_NAM_VAR_SURF(ILUOUT,'CCH_BEM',CCH_BEM,'ROW30','DOE-2')
 CALL TEST_NAM_VAR_SURF(ILUOUT,'CURB_LM',CURB_LM,'SM10','LMEZ')
 CALL TEST_NAM_VAR_SURF(ILUOUT,'CCH_DRY_DEP',CCH_DRY_DEP,'      ','WES89 ','NONE  ')
 CALL TEST_NAM_VAR_SURF(ILUOUT,'CSOLAR_PANEL',CSOLAR_PANEL,'PV ','MIX')
 !
 !* close namelist file
 !
...
!
IF (IMI.NE.-1) THEN
 CALL UPDATE_NAM_TEBn(T, TOP, TPN)
