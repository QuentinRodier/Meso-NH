...
!     ################################################################
      SUBROUTINE AV_PATCH_PGD_1D_1P (DTCO, PFIELD, PCOVER, PDATA, PIRRIG, NPAR_VEG_IRR_USE, &
                                     HSFTYPE, HATYPE, OCOVER, KMASK, KNPATCH, KPATCH, PDZ, KDECADE)
!     ################################################################
!
!
USE MODD_DATA_COVER_n, ONLY : DATA_COVER_t
...
ELSEIF (HATYPE=='MAJ' .OR. HATYPE=='MA1') THEN
  ...
  DO JI = 1,SIZE(KMASK)
    ...
    DO JVEG = 1,NVEGTYPE+NVEG_IRR
      ...
      ZXIRRIG2 = 0.
      IF (  NVEG_IRR /= 0 ) THEN
        IF ( PIRRIG(IMASK,JK) /= XUNDEF ) ZXIRRIG2 = PIRRIG(IMASK,JK)
      ENDIF
      !  
      DO JCOVER=1,ICOVER
        !
...
DO JP = 1,SIZE(TFIELD)
  ...
  DO JVEG=1,NVEGTYPE+NVEG_IRR
    ...
    IF( KPATCH == JJ ) THEN
      ...
      ZXIRRIG = 0.
      IF (  NVEG_IRR /= 0 ) THEN
        IF ( PIRRIG(IMASK,JK) /= XUNDEF ) ZXIRRIG = PIRRIG(IMASK,JK)
      ENDIF
      !
