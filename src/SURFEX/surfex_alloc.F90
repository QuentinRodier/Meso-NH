...
USE MODD_DATA_COVER_PAR,      ONLY : NVEGTYPE_ECOSG, NTILESFC, NPATCH_MAX
!
...
USE MODD_TEB_HYDRO_n, ONLY : TEB_HYDRO_INIT, TEB_HYDRO_NP_INIT 
!
...
IF (LHOOK) CALL DR_HOOK("SURFEX_ALLOC",0,ZHOOK_HANDLE)
  ...
  CALL DIAG_NP_INIT(YDSURFEX%IM%ID%ND,NPATCH_MAX)
  CALL DIAG_NP_INIT(YDSURFEX%IM%ID%NDC,NPATCH_MAX)
  CALL DIAG_EVAP_ISBA_INIT(YDSURFEX%IM%ID%DE)
  CALL DIAG_EVAP_ISBA_INIT(YDSURFEX%IM%ID%DEC)
  CALL DIAG_EVAP_ISBA_NP_INIT(YDSURFEX%IM%ID%NDE,NPATCH_MAX)
  CALL DIAG_EVAP_ISBA_NP_INIT(YDSURFEX%IM%ID%NDEC,NPATCH_MAX)
  CALL DIAG_MISC_ISBA_INIT(YDSURFEX%IM%ID%DM)
  CALL DIAG_MISC_ISBA_NP_INIT(YDSURFEX%IM%ID%NDM,NPATCH_MAX)
  CALL DIAG_UTCI_INIT(YDSURFEX%IM%ID%DU)
  !
  ...
  CALL CH_ISBA_NP_INIT(YDSURFEX%IM%NCHI,NPATCH_MAX)
  CALL GR_BIOG_INIT(YDSURFEX%IM%GB)
  CALL GR_BIOG_NP_INIT(YDSURFEX%IM%NGB,NPATCH_MAX)
  CALL SSO_INIT(YDSURFEX%IM%ISS)
  CALL SSO_NP_INIT(YDSURFEX%IM%NISS,NPATCH_MAX)
  CALL GRID_INIT(YDSURFEX%IM%G)
  CALL GRID_NP_INIT(YDSURFEX%IM%NG,NPATCH_MAX)
  CALL ISBA_K_INIT(YDSURFEX%IM%K)
  CALL ISBA_NK_INIT(YDSURFEX%IM%NK,NPATCH_MAX)
  CALL ISBA_NP_INIT(YDSURFEX%IM%NP,NPATCH_MAX)
  CALL ISBA_NPE_INIT(YDSURFEX%IM%NPE,NPATCH_MAX)
  CALL AGRI_NP_INIT(YDSURFEX%IM%NAG,NPATCH_MAX)
  !
  ...
  CALL DIAG_NP_INIT(YDSURFEX%GDM%VD%NDC,NTEB_PATCH_MAX)
  ...
  CALL ISBA_P_INIT(YDSURFEX%GDM%PHV)
  CALL ISBA_NPE_INIT(YDSURFEX%GDM%NPE,NTEB_PATCH_MAX)
  CALL ISBA_NPE_INIT(YDSURFEX%GDM%NPEHV,NTEB_PATCH_MAX)
  !
  ! TEB-GREENROOF 
  CALL DIAG_NP_INIT(YDSURFEX%GRM%VD%ND,NTEB_PATCH_MAX)
  CALL DIAG_NP_INIT(YDSURFEX%GRM%VD%NDC,NTEB_PATCH_MAX)
  ...
  CALL DIAG_UTCI_INIT(YDSURFEX%TM%TD%DU)
  ! 
  ...
  CALL DATA_TEB_HYDRO_INIT(YDSURFEX%HM%DTH)
  CALL TEB_HYDRO_OPTIONS_INIT(YDSURFEX%HM%THO)
  CALL TEB_HYDRO_PGD_INIT(YDSURFEX%HM%THP)
  CALL TEB_HYDRO_NP_INIT(YDSURFEX%HM%NTH,NTEB_PATCH_MAX)
  !
  ...
  CALL DST_NP_INIT(YDSURFEX%NDST,NPATCH_MAX)
