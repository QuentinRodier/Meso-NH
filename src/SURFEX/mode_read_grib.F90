...
!
 CALL MAKE_GRIB_INDEX(HGRIB,HINMODEL)
!
IFOUND=0
KRET=0
!
...
IF (PRESENT(KPARAM2)) THEN
  IF (IFOUND/=3) THEN
    ...
    IF (KRET.EQ.0) THEN
      ILTYPE=-2
      IF (PRESENT(KLTYPE)) ILTYPE=KLTYPE
      IF (PRESENT(KLTYPE2)) ILTYPE=KLTYPE2
      CALL GET_GRIB_MESSAGE(KLUOUT,ILTYPE,ILEV1,ILEV2,IGRIB,IFOUND)
    ENDIF
  ELSE
    KPARAM2 = KPARAM
...
INTEGER                           :: ILTYPE,ILTYPE2    ! leveltype
INTEGER                           :: ILEV      ! level
...
ILEV1 = -1
ILEV2 = -1
 CALL READ_GRIB(HGRIB,HINMODEL,KLUOUT,183,IRET,ZFIELD,KLTYPE=ILTYPE,KLEV1=ILEV1,KLEV2=ILEV2)
!
IF (IRET == 0) THEN      
...
ILEV1 = -1
ILEV2 = -1
 CALL READ_GRIB(HGRIB,HINMODEL,KLUOUT,170,IRET,ZFIELD,KLTYPE=ILTYPE,KLEV1=ILEV1,KLEV2=ILEV2)
!
IF (IRET== 0) THEN         
...
ILEV2 = -1
IPAR=39
 CALL READ_GRIB(HGRIB,HINMODEL,KLUOUT,140,IRET,ZFIELD,KLTYPE=ILTYPE,KLEV1=ILEV1,KLEV2=ILEV2,KPARAM2=IPAR)
!
IF (IRET == 0) THEN 
...
ILEV2 = -1
IPAR=42
 CALL READ_GRIB(HGRIB,HINMODEL,KLUOUT,237,IRET,ZFIELD,KLTYPE=ILTYPE,KLEV1=ILEV1,KLEV2=ILEV2,KPARAM2=IPAR)
!
IF (IRET == 0) THEN   
  INLAYERDEEP = 4
...
ILEV2 = -1
IPAR=41
 CALL READ_GRIB(HGRIB,HINMODEL,KLUOUT,184,IRET,ZFIELD,KLTYPE=ILTYPE,KLEV1=ILEV1,KLEV2=ILEV2,KPARAM2=IPAR)
!
IF (IRET == 0) THEN
...
ILEV2 = -1
IPAR=40
 CALL READ_GRIB(HGRIB,HINMODEL,KLUOUT,171,IRET,ZFIELD,KLTYPE=ILTYPE,KLEV1=ILEV1,KLEV2=ILEV2,KPARAM2=IPAR)
!
IF (IRET == 0) THEN
...
!     ----------------------------
 CALL READ_GRIB(HGRIB,HINMODEL,KLUOUT,43,IRET,ZSLT)
!--------------------------------------------------------------------------------
...
IF (HINMODEL == 'ALADIN') IPAR=128
 CALL READ_GRIB(HGRIB,HINMODEL,KLUOUT,IPAR,IRET,PCLAY)
!
...
IF (HINMODEL == 'ALADIN') IPAR=129
 CALL READ_GRIB(HGRIB,HINMODEL,KLUOUT,IPAR,IRET,PSAND)
!
! if not available, the model is not ISBA (IWMODE=1)
IF (GISBA) THEN
  IF (IRET /= 0) THEN
...
!    Gaelle Delautier (via Q.Rodier) 01/2019 : add GRIB 2 
!
USE MODD_GRID_GRIB,  ONLY : NNI,NGRIB_VERSION
USE MODD_SURF_PAR,   ONLY : XUNDEF
!
IMPLICIT NONE
!
...
IF (HINMODEL == 'ARPEGE' .OR. HINMODEL=='MOCAGE') THEN
  ...
  ELSE
    IPARAM=153
    ILEV2   = 1 
    CALL READ_GRIB(HGRIB,HINMODEL,KLUOUT,IPARAM,IRET,ZFIELD,KLEV1=ILEV1,KLEV2=ILEV2)
  ENDIF
ELSE
  ILTYPE  = 105
  ILEV2   = 0
  CALL READ_GRIB(HGRIB,HINMODEL,KLUOUT,86,IRET,ZFIELD,KLEV1=ILEV1,KLEV2=ILEV2)
ENDIF
IF (IRET /= 0) THEN
...
IF (GISBA) THEN
  IF (HINMODEL == 'ARPEGE' .OR. HINMODEL == 'MOCAGE') THEN
    CALL READ_GRIB(HGRIB,HINMODEL,KLUOUT,173,IRET,ZFIELD)
  ELSE
    CALL READ_GRIB(HGRIB,HINMODEL,KLUOUT,130,IRET,ZFIELD)
  ENDIF
  IF (IRET /= 0) THEN
...
IF (HINMODEL == 'ARPEGE' .OR. HINMODEL=='MOCAGE') THEN
  ...
  ELSE
    ...
    ILTYPE  = 112     
    ILEV2   = 1
    CALL READ_GRIB(HGRIB,HINMODEL,KLUOUT,IPARAM,IRET,ZFIELD,KLTYPE=ILTYPE,KLEV1=ILEV1,KLEV2=ILEV2)
  ENDIF
ELSE
  ILTYPE  = 105      
  ILEV2   = 0        
...
ILEV1 = 904
ILEV2 = -1
 CALL READ_GRIB(HGRIB,HINMODEL,KLUOUT,11,IRET,ZFIELD,KLEV1=ILEV1,KLEV2=ILEV2)
IF (IRET /= 0 ) THEN
...
ILEV1=904
ILEV2=-1
 CALL READ_GRIB(HGRIB,HINMODEL,KLUOUT,86,IRET,ZFIELD,KLTYPE=ILTYPE,KLEV1=ILEV1,KLEV2=ILEV2)
IF (IRET /= 0 ) THEN
...
SELECT CASE(HINMODEL)
  ...
  CASE('ARPEGE','ALADIN','MOCAGE','HIRLAM')
    ...
    ENDIF
  CASE('NCEP  ')
    CALL READ_GRIB(HGRIB,HINMODEL,KLUOUT,3066,IRET,ZFIELD)    
  CASE DEFAULT
...
IF (HINMODEL == 'ECMWF') THEN
  CALL READ_GRIB(HGRIB,HINMODEL,KLUOUT,32,IRET,ZFIELD)
  IF (IRET == 0 ) THEN
    DEALLOCATE(PSNVA)      
...
PSNV(:) = XRHOSMAX
IF (HINMODEL == 'ECMWF') THEN
  CALL READ_GRIB(HGRIB,HINMODEL,KLUOUT,33,IRET,ZFIELD)
  IF (IRET == 0 ) THEN
    DEALLOCATE(PSNV)      
    ALLOCATE(PSNV(SIZE(ZFIELD)))
    PSNV(:)=ZFIELD(:)
...
ZLEV1= 0.0
ZLEV2= 0.1
CALL READ_GRIB(HGRIB,HINMODEL,KLUOUT,228139,IRET,ZFIELD,KLTYPE=ILTYPE,KLEV1=ILEV1,KLEV2=ILEV2,&
               HTYPELEVEL=YTYPELEVEL,PLEV1=ZLEV1,PLEV2=ZLEV2)
!
IF (IRET== 0) THEN
  ZD(1)=(ZLEV2 - ZLEV1)
...
ZLEV1= 0.0
ZLEV2= 0.1
CALL READ_GRIB(HGRIB,HINMODEL,KLUOUT,260185,IRET,ZFIELD,KLTYPE=ILTYPE,KLEV1=ILEV1,KLEV2=ILEV2,&
               HTYPELEVEL=YTYPELEVEL,PLEV1=ZLEV1,PLEV2=ZLEV2)
!
IF (IRET== 0) THEN
  ZD(1)=(ZLEV2 - ZLEV1)
