...
SUBROUTINE INI_VAR_FROM_DATA_NAT_1D (DTCO, UG, U, USS, PPAR_VEGTYPE, &
                                 HPROGRAM, HATYPE, HNAME, HTYPE, HFNAM, HFTYP, PUNIF, PFIELD, OPRESENT, LAUTO_C)
!
...
!
      SUBROUTINE INI_VAR_FROM_DATA_NAT_2D (DTCO, UG, U, USS, PPAR_VEGTYPE, &
                                       HPROGRAM, HATYPE, HNAME, HTYPE, HFNAM, HFTYP, PUNIF, PFIELD_TIME, OPRESENT, LAUTO_C)
!
USE MODD_DATA_COVER_n,    ONLY : DATA_COVER_t
USE MODD_SURF_ATM_GRID_n, ONLY : SURF_ATM_GRID_t
USE MODD_SURF_ATM_n,      ONLY : SURF_ATM_t
...
ELSE
  ...
  ENDIF
  !
  IF (LPAUTO_C) THEN ! #autoimplement. Possible to desactivate
                     !                 -> because use to complete "no data" information... why ?? ==> problem with irrigation
