...
!     #########
      SUBROUTINE READ_DEFAULT_TEB_n (CHT, DMTO, DGO, DUT, IO, T, TOP, TPN, HPROGRAM)
!     #######################################################
!
...
IF (LNAM_READ) THEN
 ...
 CALL POSNAM(ILUDES,'NAM_TEB_PANELN',GFOUND,ILUOUT)
 IF (GFOUND) READ(UNIT=ILUDES,NML=NAM_TEB_PANELn)
 !
ENDIF
!
IF (IMI.NE.-1) THEN
