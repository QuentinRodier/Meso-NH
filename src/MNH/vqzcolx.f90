...
INTERFACE
!
  SUBROUTINE VQZCOLX (KND, PALPHAX, PNUX, PALPHAZ, PNUZ,                  &
                      PEXZ, PEXMASSZ, PFALLX, PEXFALLX, PFALLZ, PEXFALLZ, &
                      PEFALL, PLBDAXMAX, PLBDAZMAX, PLBDAXMIN, PLBDAZMIN, &
                      PDINFTY, PRZCOLX                                    )
!
...
REAL, INTENT(IN) :: PEFALL    ! Exponent of the fall speed difference
...
!
  END SUBROUTINE VQZCOLX
END INTERFACE
END MODULE MODI_VQZCOLX
!
!     #########################################################################
      SUBROUTINE VQZCOLX (KND, PALPHAX, PNUX, PALPHAZ, PNUZ,                  &
                          PEXZ, PEXMASSZ, PFALLX, PEXFALLX, PFALLZ, PEXFALLZ, &
...
!!           -- adapted for electricity (non-inductive charging)
!!
!!
!!    PURPOSE
!!    -------
...
!!       / /
!!       |{|                                                 }
!!       |{| E_xz (Dx+Dz)^2 |cxDx^dx-czDz^dz|^n Dz^m g(Dz) dDz} g(Dx) dDx
!!       |{|                                                 }
!!       / /
!!        0
!!
...
!!       / /
!!       |{|                          }
!!       |{| (Dx+Dz)^2 Dz^m g(Dz) dDz} g(Dx) dDx
!!       |{|                          }
!!       / /
!!        0
!!
...
!
        ZCOLLZ = ZCOLLZ + ZFUNC * PEXZ   &
                 * (ABS(PFALLX*ZDX**PEXFALLX-PFALLZ*ZDZ**PEXFALLZ))**PEFALL
      END DO
!
