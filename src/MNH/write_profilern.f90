...
PROFILER: DO JS = 1, INUMPROF
  ...
  ELSE
    ...
    ELSE IF ( ISP == TPDIAFILE%NMASTER_RANK ) THEN
      ...
      TZPROFILER%XZON(:,:) = RESHAPE( ZPACK(IPOS:IPOS+ISTORE*IKU-1), [ IKU, ISTORE ] ) ; IPOS = IPOS + ISTORE * IKU
      TZPROFILER%XMER(:,:) = RESHAPE( ZPACK(IPOS:IPOS+ISTORE*IKU-1), [ IKU, ISTORE ] ) ; IPOS = IPOS + ISTORE * IKU
      TZPROFILER%XFF(:,:)  = RESHAPE( ZPACK(IPOS:IPOS+ISTORE*IKU-1), [ IKU, ISTORE ] ) ; IPOS = IPOS + ISTORE * IKU
      TZPROFILER%XDD(:,:)  = RESHAPE( ZPACK(IPOS:IPOS+ISTORE*IKU-1), [ IKU, ISTORE ] ) ; IPOS = IPOS + ISTORE * IKU
      TZPROFILER%XW(:,:)   = RESHAPE( ZPACK(IPOS:IPOS+ISTORE*IKU-1), [ IKU, ISTORE ] ) ; IPOS = IPOS + ISTORE * IKU
      TZPROFILER%XP(:,:)   = RESHAPE( ZPACK(IPOS:IPOS+ISTORE*IKU-1), [ IKU, ISTORE ] ) ; IPOS = IPOS + ISTORE * IKU
      ...
      IF ( CCLOUD == 'C2R2' .OR. CCLOUD == 'KHKO' ) THEN
        TZPROFILER%XVISIGUL(:,:) = RESHAPE( ZPACK(IPOS:IPOS+ISTORE*IKU-1), [ IKU, ISTORE ] ) ; IPOS = IPOS + ISTORE * IKU
      END IF
      IF ( CCLOUD /= 'NONE' .AND. CCLOUD /= 'REVE' ) THEN
        TZPROFILER%XVISIKUN(:,:) = RESHAPE( ZPACK(IPOS:IPOS+ISTORE*IKU-1), [ IKU, ISTORE ] ) ; IPOS = IPOS + ISTORE * IKU
      END IF
      TZPROFILER%XCRARE(:,:)     = RESHAPE( ZPACK(IPOS:IPOS+ISTORE*IKU-1), [ IKU, ISTORE ] ) ; IPOS = IPOS + ISTORE * IKU
      TZPROFILER%XCRARE_ATT(:,:) = RESHAPE( ZPACK(IPOS:IPOS+ISTORE*IKU-1), [ IKU, ISTORE ] ) ; IPOS = IPOS + ISTORE * IKU
      IF ( CCLOUD == 'ICE3' .OR. CCLOUD == 'ICE4' ) THEN
        TZPROFILER%XCIZ(:,:) = RESHAPE( ZPACK(IPOS:IPOS+ISTORE*IKU-1), [ IKU, ISTORE ] ) ; IPOS = IPOS + ISTORE * IKU
      END IF
      TZPROFILER%XLWCZ(:,:) = RESHAPE( ZPACK(IPOS:IPOS+ISTORE*IKU-1), [ IKU, ISTORE ] ) ; IPOS = IPOS + ISTORE * IKU
      TZPROFILER%XIWCZ(:,:) = RESHAPE( ZPACK(IPOS:IPOS+ISTORE*IKU-1), [ IKU, ISTORE ] ) ; IPOS = IPOS + ISTORE * IKU
      TZPROFILER%XRHOD(:,:) = RESHAPE( ZPACK(IPOS:IPOS+ISTORE*IKU-1), [ IKU, ISTORE ] ) ; IPOS = IPOS + ISTORE * IKU

      TZPROFILER%XR(:,:,:)   = RESHAPE( ZPACK(IPOS:IPOS+ISTORE*IKU*NRR-1),  [ IKU, ISTORE, NRR ] )
      IPOS = IPOS + ISTORE * IKU * NRR
      TZPROFILER%XSV(:,:,:)  = RESHAPE( ZPACK(IPOS:IPOS+ISTORE*IKU*NSV-1),  [ IKU, ISTORE, NSV ] )
      IPOS = IPOS + ISTORE * IKU * NSV
      TZPROFILER%XAER(:,:,:) = RESHAPE( ZPACK(IPOS:IPOS+ISTORE*IKU*NAER-1), [ IKU, ISTORE, NAER ] )
      IPOS = IPOS + ISTORE * IKU * NAER
      ...
      TZPROFILER%XTKE_DISS(:,:) = RESHAPE( ZPACK(IPOS:IPOS+ISTORE*IKU-1), [ IKU, ISTORE ] ) ; IPOS = IPOS + ISTORE * IKU
