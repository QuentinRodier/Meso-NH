...
USE MODE_MSG
USE MODE_STATPROF_TOOLS,   ONLY: STATPROF_INSTANT, STATPROF_INTERP_2D, &
...
PROFILER: DO JP = 1, NUMBPROFILER_LOC
  ZZ(:)                  = TPROFILERS(JP)%INTERP_FROM_MASSPOINT_VERT( PZ )
  ZRHOD(:)               = TPROFILERS(JP)%INTERP_FROM_MASSPOINT_VERT( PRHODREF )
  ZPRES(:)               = TPROFILERS(JP)%INTERP_FROM_MASSPOINT_VERT( PP )
  ...
  IF ( ABS( ZZ(IKB)-XZS_GPS ) < 150 ) THEN ! distance between real and model orography ok
    ZRV(:)                 = TPROFILERS(JP)%INTERP_FROM_MASSPOINT_VERT( PR(:,:,:,1) )
    ZT(:)                  = TPROFILERS(JP)%INTERP_FROM_MASSPOINT_VERT( ZTEMP )
    ZE(:)                  = ZPRES(:)*ZRV(:)/(ZRDSRV+ZRV(:))
    ZTV(:)                 = TPROFILERS(JP)%INTERP_FROM_MASSPOINT_VERT( ZTEMPV )
...
! add contribution below the model orography
  ...
  TPROFILERS(JP)%XW   (IN,:) = TPROFILERS(JP)%INTERP_FROM_MASSPOINT_VERT( PW )
  TPROFILERS(JP)%XTH  (IN,:) = TPROFILERS(JP)%INTERP_FROM_MASSPOINT_VERT( PTH )
  TPROFILERS(JP)%XTHV (IN,:) = TPROFILERS(JP)%INTERP_FROM_MASSPOINT_VERT( ZTHV )
  IF ( CCLOUD == 'C2R2' .OR. CCLOUD == 'KHKO' ) &
    TPROFILERS(JP)%XVISIGUL(IN,:) = TPROFILERS(JP)%INTERP_FROM_MASSPOINT_VERT( ZVISIGUL )
  IF ( CCLOUD /= 'NONE' .AND. CCLOUD /= 'REVE' ) &
    TPROFILERS(JP)%XVISIKUN(IN,:) = TPROFILERS(JP)%INTERP_FROM_MASSPOINT_VERT( ZVISIKUN )
  TPROFILERS(JP)%XZZ  (IN,:) = ZZ(:)
  TPROFILERS(JP)%XRHOD(IN,:) = ZRHOD(:)
  IF ( CCLOUD == 'ICE3' .OR. CCLOUD == 'ICE4' ) &
    TPROFILERS(JP)%XCIZ(IN,:) = TPROFILERS(JP)%INTERP_FROM_MASSPOINT_VERT( PCIT )
! add RARE
  ...
  IF (CCLOUD=="LIMA" .OR. CCLOUD=="ICE3" ) THEN ! only for ICE3 and LIMA
    TPROFILERS(JP)%XLWCZ(IN,:) = TPROFILERS(JP)%INTERP_FROM_MASSPOINT_VERT( (PR(:,:,:,2)+PR(:,:,:,3))*PRHODREF(:,:,:) )
    TPROFILERS(JP)%XIWCZ(IN,:) = TPROFILERS(JP)%INTERP_FROM_MASSPOINT_VERT( (PR(:,:,:,4)+PR(:,:,:,5)+PR(:,:,:,6))*PRHODREF(:,:,:) )
    ZTEMPZ(:)=TPROFILERS(JP)%INTERP_FROM_MASSPOINT_VERT( ZTEMP(:,:,:) )
    ZRHODREFZ(:)=TPROFILERS(JP)%INTERP_FROM_MASSPOINT_VERT( PRHODREF(:,:,:) )
    ZCIT(:)=TPROFILERS(JP)%INTERP_FROM_MASSPOINT_VERT( PCIT(:,:,:) )
    IF (CCLOUD=="LIMA") THEN
      ZCCI(:)=TPROFILERS(JP)%INTERP_FROM_MASSPOINT_VERT( PSV(:,:,:,NSV_LIMA_NI) )
      ZCCR(:)=TPROFILERS(JP)%INTERP_FROM_MASSPOINT_VERT( PSV(:,:,:,NSV_LIMA_NR) )
      ZCCC(:)=TPROFILERS(JP)%INTERP_FROM_MASSPOINT_VERT( PSV(:,:,:,NSV_LIMA_NC) )
    END IF
    DO JLOOP=3,6
      ZRZ(:,JLOOP)=TPROFILERS(JP)%INTERP_FROM_MASSPOINT_VERT( PR(:,:,:,JLOOP) )
    END DO
    IF (CSURF=="EXTE") THEN
      DO JK=1,IKU
      ...
      END DO
    ELSE
      ZRZ(:,2)=TPROFILERS(JP)%INTERP_FROM_MASSPOINT_VERT( PR(:,:,:,2) )
      ZRZ(:,7)=0.
    END IF
    ALLOCATE(ZAELOC(IKU))
    !
 ...
 ! apply attenuation
    ALLOCATE(ZZMZ(IKU))
    ZZMZ = ZZ(:) ! TPROFILERS(JP)%INTERP_FROM_MASSPOINT_VERT( ZZM(:,:,:) )
!        ZZMZ(1)=ZZM_STAT
    ! zenith
    ZAETOT=1.
    DO JK = 2,IKU
...
! end add RARE
!!
  TPROFILERS(JP)%XP   (IN,:) = TPROFILERS(JP)%INTERP_FROM_MASSPOINT_VERT( PP )
  !
  DO JSV=1,SIZE(PR,4)
    TPROFILERS(JP)%XR   (IN,:,JSV) = TPROFILERS(JP)%INTERP_FROM_MASSPOINT_VERT( PR(:,:,:,JSV) )
  END DO
    ZWORK(:,:,:,:)=PSV(:,:,:,:)
    ZWORK(:,:,1,:)=PSV(:,:,2,:)
  DO JSV=1,SIZE(PSV,4)
    TPROFILERS(JP)%XSV  (IN,:,JSV) = TPROFILERS(JP)%INTERP_FROM_MASSPOINT_VERT( ZWORK(:,:,:,JSV) )
  END DO
  ZWORK2(:,:,:,:) = 0.
  DO JK=IKB,IKE
    IKRAD = JK - JPVEXT
  ...
  DO JSV=1,SIZE(PAER,4)
    TPROFILERS(JP)%XAER(IN,:,JSV) = TPROFILERS(JP)%INTERP_FROM_MASSPOINT_VERT( ZWORK2(:,:,:,JSV) )
  END DO
  IF (SIZE(PTKE)>0) TPROFILERS(JP)%XTKE  (IN,:) = TPROFILERS(JP)%INTERP_FROM_MASSPOINT_VERT( PTKE )
  !
  ...
  END IF
  TPROFILERS(JP)%XTKE_DISS(IN,:) = TPROFILERS(JP)%INTERP_FROM_MASSPOINT_VERT( XCURRENT_TKE_DISS )
END DO PROFILER
!
!----------------------------------------------------------------------------
!
