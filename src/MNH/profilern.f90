...
! add RARE
  ...
  IF (CCLOUD=="LIMA" .OR. CCLOUD=="ICE3" ) THEN ! only for ICE3 and LIMA
    TPROFILERS(JP)%XLWCZ(IN,:) = TPROFILERS(JP)%INTERP_HOR_FROM_MASSPOINT( (PR(:,:,:,2)+PR(:,:,:,3))*PRHODREF(:,:,:) )
    TPROFILERS(JP)%XIWCZ(IN,:) = &
      TPROFILERS(JP)%INTERP_HOR_FROM_MASSPOINT( (PR(:,:,:,4)+PR(:,:,:,5)+PR(:,:,:,6))*PRHODREF(:,:,:) )
    ZTEMPZ(:)=TPROFILERS(JP)%INTERP_HOR_FROM_MASSPOINT( ZTEMP(:,:,:) )
    ZRHODREFZ(:)=TPROFILERS(JP)%INTERP_HOR_FROM_MASSPOINT( PRHODREF(:,:,:) )
    IF (CCLOUD=="LIMA") THEN
      ZCCI(:)=TPROFILERS(JP)%INTERP_HOR_FROM_MASSPOINT( PSV(:,:,:,NSV_LIMA_NI) )
      ZCCR(:)=TPROFILERS(JP)%INTERP_HOR_FROM_MASSPOINT( PSV(:,:,:,NSV_LIMA_NR) )
      ZCCC(:)=TPROFILERS(JP)%INTERP_HOR_FROM_MASSPOINT( PSV(:,:,:,NSV_LIMA_NC) )
    ELSE
      ZCIT(:)=TPROFILERS(JP)%INTERP_HOR_FROM_MASSPOINT( PCIT(:,:,:) )
    END IF
    DO JLOOP=3,6
      ZRZ(:,JLOOP)=TPROFILERS(JP)%INTERP_HOR_FROM_MASSPOINT( PR(:,:,:,JLOOP) )
    END DO
    IF (CSURF=="EXTE") THEN
      DO JK=1,IKU
        ZRZ(JK,2)=TPROFILERS(JP)%INTERP_HOR_FROM_MASSPOINT( PR(:,:,JK,2)*PSEA(:,:) )       ! becomes cloud mixing ratio over sea
        ZRZ(JK,7)=TPROFILERS(JP)%INTERP_HOR_FROM_MASSPOINT( PR(:,:,JK,2)*(1.-PSEA(:,:)) )  ! becomes cloud mixing ratio over land
      END DO
    ELSE
      ZRZ(:,2)=TPROFILERS(JP)%INTERP_HOR_FROM_MASSPOINT( PR(:,:,:,2) )
      ZRZ(:,7)=0.
    END IF
    ALLOCATE(ZAELOC(IKU))
    !
 ...
 ! apply attenuation
    ALLOCATE(ZZMZ(IKU))
    ZZMZ = ZZ(:) ! TPROFILERS(JP)%INTERP_HOR_FROM_MASSPOINT( ZZM(:,:,:) )
!        ZZMZ(1)=ZZM_STAT
    ! zenith
    ZAETOT=1.
    DO JK = 2,IKU
...
!!
  ...
  IF ( LDIAG_SURFRAD_PROF ) THEN
    TPROFILERS(JP)%XT2M   (IN)     = TPROFILERS(JP)%INTERP_HOR_FROM_MASSPOINT( XCURRENT_T2M    )
    TPROFILERS(JP)%XQ2M   (IN)     = TPROFILERS(JP)%INTERP_HOR_FROM_MASSPOINT( XCURRENT_Q2M    )
    TPROFILERS(JP)%XHU2M  (IN)     = TPROFILERS(JP)%INTERP_HOR_FROM_MASSPOINT( XCURRENT_HU2M   )
    TPROFILERS(JP)%XZON10M(IN)     = TPROFILERS(JP)%INTERP_HOR_FROM_MASSPOINT( XCURRENT_ZON10M )
    TPROFILERS(JP)%XMER10M(IN)     = TPROFILERS(JP)%INTERP_HOR_FROM_MASSPOINT( XCURRENT_MER10M )
    TPROFILERS(JP)%XRN    (IN)     = TPROFILERS(JP)%INTERP_HOR_FROM_MASSPOINT( XCURRENT_RN     )
    TPROFILERS(JP)%XH     (IN)     = TPROFILERS(JP)%INTERP_HOR_FROM_MASSPOINT( XCURRENT_H      )
    TPROFILERS(JP)%XLE    (IN)     = TPROFILERS(JP)%INTERP_HOR_FROM_MASSPOINT( XCURRENT_LE     )
    TPROFILERS(JP)%XLEI   (IN)     = TPROFILERS(JP)%INTERP_HOR_FROM_MASSPOINT( XCURRENT_LEI    )
    TPROFILERS(JP)%XGFLUX (IN)     = TPROFILERS(JP)%INTERP_HOR_FROM_MASSPOINT( XCURRENT_GFLUX  )
    IF (CRAD /= 'NONE') THEN
      TPROFILERS(JP)%XSWD   (IN)     = TPROFILERS(JP)%INTERP_HOR_FROM_MASSPOINT( XCURRENT_SWD    )
      TPROFILERS(JP)%XSWU   (IN)     = TPROFILERS(JP)%INTERP_HOR_FROM_MASSPOINT( XCURRENT_SWU    )
      TPROFILERS(JP)%XLWD   (IN)     = TPROFILERS(JP)%INTERP_HOR_FROM_MASSPOINT( XCURRENT_LWD    )
      TPROFILERS(JP)%XLWU   (IN)     = TPROFILERS(JP)%INTERP_HOR_FROM_MASSPOINT( XCURRENT_LWU    )
      TPROFILERS(JP)%XSWDIR (IN)     = TPROFILERS(JP)%INTERP_HOR_FROM_MASSPOINT( XCURRENT_SWDIR  )
      TPROFILERS(JP)%XSWDIFF(IN)     = TPROFILERS(JP)%INTERP_HOR_FROM_MASSPOINT( XCURRENT_SWDIFF )
      TPROFILERS(JP)%XDSTAOD(IN)     = TPROFILERS(JP)%INTERP_HOR_FROM_MASSPOINT( XCURRENT_DSTAOD )
      TPROFILERS(JP)%XSLTAOD(IN)     = TPROFILERS(JP)%INTERP_HOR_FROM_MASSPOINT( XCURRENT_SLTAOD )
    END IF
    TPROFILERS(JP)%XSFCO2   (IN)   = TPROFILERS(JP)%INTERP_HOR_FROM_MASSPOINT( XCURRENT_SFCO2    )
  END IF
