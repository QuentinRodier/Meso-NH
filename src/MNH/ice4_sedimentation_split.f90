...
USE MODD_RAIN_ICE_DESCR, ONLY: XCC,XCEXVT,XDC,XLBEXC,XRTMIN,XALPHAS,XNUS,XBS,XFVELOS, &
                               XLBDAS_MAX,XLBDAS_MIN,XLBEXS,XLBS,XTRANS_MP_GAMMAS
      ...
      DO JL=1, ISEDIM
        ...
        IF(PRXT(JI,JJ,JK)> XRTMIN(KSPE)) THEN
           IF (LSNOW_T .AND. PT(JI,JJ,JK)>263.15) THEN
              ZLBDA = MAX(MIN(XLBDAS_MAX, 10**(14.554-0.0423*PT(JI,JJ,JK))),XLBDAS_MIN)*XTRANS_MP_GAMMAS
           ELSE IF (LSNOW_T) THEN
              ZLBDA = MAX(MIN(XLBDAS_MAX, 10**(6.226 -0.0106*PT(JI,JJ,JK))),XLBDAS_MIN)*XTRANS_MP_GAMMAS
           ELSE
