...
STATION: DO JS = 1, INUMSTAT
  ...
  ELSE
    ...
    IF ( ISP == ISTATPRCRANK(JS) ) THEN
      ...
      ZPACK(IPOS) = TSTATIONS(IDX)%XX_CUR;   IPOS = IPOS + 1
      ZPACK(IPOS) = TSTATIONS(IDX)%XY_CUR;   IPOS = IPOS + 1
      ZPACK(IPOS) = TSTATIONS(IDX)%XZ_CUR;   IPOS = IPOS + 1
      ZPACK(IPOS) = TSTATIONS(IDX)%XLON_CUR; IPOS = IPOS + 1
      ZPACK(IPOS) = TSTATIONS(IDX)%XLAT_CUR; IPOS = IPOS + 1
    ...
    ELSE IF ( ISP == TPDIAFILE%NMASTER_RANK ) THEN
      ...
      TZSTATION%XX_CUR   = ZPACK(IPOS);         IPOS = IPOS + 1
      TZSTATION%XY_CUR   = ZPACK(IPOS);         IPOS = IPOS + 1
      TZSTATION%XZ_CUR   = ZPACK(IPOS);         IPOS = IPOS + 1
      TZSTATION%XLON_CUR = ZPACK(IPOS);         IPOS = IPOS + 1
      TZSTATION%XLAT_CUR = ZPACK(IPOS);         IPOS = IPOS + 1
