...
TYPE, EXTENDS(TSENSOR), ABSTRACT :: TFLYERDATA
  ...
  CONTAINS
    PROCEDURE :: DATA_ARRAYS_ALLOCATE   => FLYER_ALLOCATE
    PROCEDURE :: DATA_ARRAYS_DEALLOCATE => FLYER_DEALLOCATE
END TYPE TFLYERDATA
...
CONTAINS
SUBROUTINE FLYER_ALLOCATE( TPSENSOR, KSTORE )
  ...
  CALL PRINT_MSG( NVERB_DEBUG, 'GEN', 'FLYER_ALLOCATE', 'flyer: ' // TRIM(TPSENSOR%CNAME), OLOCAL = .TRUE. )
  ...
  ALLOCATE( TPSENSOR%XZON (1, ISTORE) )
  ALLOCATE( TPSENSOR%XMER (1, ISTORE) )
  ALLOCATE( TPSENSOR%XW   (1, ISTORE) )
  ALLOCATE( TPSENSOR%XP   (1, ISTORE) )
  ALLOCATE( TPSENSOR%XTH  (1, ISTORE) )
  ALLOCATE( TPSENSOR%XR   (1, ISTORE, NRR) )
  ALLOCATE( TPSENSOR%XSV  (1, ISTORE, NSV) )
  ALLOCATE( TPSENSOR%XRTZ (IKU, ISTORE) )
  ALLOCATE( TPSENSOR%XRZ  (IKU, ISTORE, NRR) )
  ALLOCATE( TPSENSOR%XFFZ (IKU, ISTORE) )
  ALLOCATE( TPSENSOR%XIWCZ(IKU, ISTORE) )
  ALLOCATE( TPSENSOR%XLWCZ(IKU, ISTORE) )
  ...
  ENDIF
  ALLOCATE( TPSENSOR%XCRARE    (IKU, ISTORE) )
  ALLOCATE( TPSENSOR%XCRARE_ATT(IKU, ISTORE) )
  ALLOCATE( TPSENSOR%XWZ       (IKU, ISTORE) )
  ALLOCATE( TPSENSOR%XZZ       (IKU, ISTORE) )
  IF ( CTURB == 'TKEL' ) THEN
    ALLOCATE( TPSENSOR%XTKE(1, ISTORE) )
  ELSE
    ALLOCATE( TPSENSOR%XTKE(0, 0) )
  END IF
  ALLOCATE( TPSENSOR%XTKE_DISS(ISTORE) )
  IF ( CRAD /= 'NONE' ) THEN
    ALLOCATE( TPSENSOR%XTSRAD(1, ISTORE) )
  ELSE
    ALLOCATE( TPSENSOR%XTSRAD(0, 0) )
  END IF
...
END SUBROUTINE FLYER_ALLOCATE

SUBROUTINE FLYER_DEALLOCATE( TPSENSOR )
  USE MODE_MSG
  ...
  CALL PRINT_MSG( NVERB_DEBUG, 'GEN', 'FLYER_DEALLOCATE', 'flyer: ' // TRIM(TPSENSOR%CNAME), OLOCAL = .TRUE. )
...
END SUBROUTINE FLYER_DEALLOCATE
