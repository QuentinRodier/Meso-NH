...
!
TPFLYER%XW   (ISTORE) = TPFLYER%INTERP_FROM_MASSPOINT( ZWM )
TPFLYER%XTH  (ISTORE) = TPFLYER%INTERP_FROM_MASSPOINT( PTH )
!
...
DO JLOOP=1,SIZE(PR,4)
  TPFLYER%XR   (ISTORE,JLOOP) = TPFLYER%INTERP_FROM_MASSPOINT( PR(:,:,:,JLOOP) )
  IF (JLOOP>=2) ZR(:,:,:) = ZR(:,:,:) + PR(:,:,:,JLOOP)
END DO
DO JLOOP=1,SIZE(PSV,4)
  TPFLYER%XSV  (ISTORE,JLOOP) = TPFLYER%INTERP_FROM_MASSPOINT( PSV(:,:,:,JLOOP) )
END DO
...
IF (CCLOUD=="LIMA" .OR. CCLOUD=="ICE3" ) THEN ! only for ICE3 and LIMA
  TPFLYER%XLWCZ  (ISTORE,:) = TPFLYER%INTERP_HOR_FROM_MASSPOINT( (PR(:,:,:,2)+PR(:,:,:,3))*PRHODREF(:,:,:) )
  TPFLYER%XIWCZ  (ISTORE,:) = TPFLYER%INTERP_HOR_FROM_MASSPOINT( (PR(:,:,:,4)+PR(:,:,:,5)+PR(:,:,:,6))*PRHODREF(:,:,:) )
  ZTEMPZ(:)=TPFLYER%INTERP_HOR_FROM_MASSPOINT( PTH(II_M:II_M+1,IJ_M:IJ_M+1,:) * ZEXN(:,:,:) )
  ZRHODREFZ(:)=TPFLYER%INTERP_HOR_FROM_MASSPOINT( PRHODREF(:,:,:) )
  IF (CCLOUD=="LIMA") THEN
    ZCCI(:)=TPFLYER%INTERP_HOR_FROM_MASSPOINT( PSV(:,:,:,NSV_LIMA_NI) )
    ZCCR(:)=TPFLYER%INTERP_HOR_FROM_MASSPOINT( PSV(:,:,:,NSV_LIMA_NR) )
    ZCCC(:)=TPFLYER%INTERP_HOR_FROM_MASSPOINT( PSV(:,:,:,NSV_LIMA_NC) )
  ELSE
    ZCIT(:)=TPFLYER%INTERP_HOR_FROM_MASSPOINT( PCIT(:,:,:) )
  ENDIF
  DO JLOOP=3,6
    ZRZ(:,JLOOP)=TPFLYER%INTERP_HOR_FROM_MASSPOINT( PR(:,:,:,JLOOP) )
  END DO
  if ( csurf == 'EXTE' ) then
    DO JK=1,IKU
      ZRZ(JK,2)=TPFLYER%INTERP_HOR_FROM_MASSPOINT( PR(:,:,JK,2)*PSEA(:,:) )       ! becomes cloud mixing ratio over sea
      ZRZ(JK,7)=TPFLYER%INTERP_HOR_FROM_MASSPOINT( PR(:,:,JK,2)*(1.-PSEA(:,:)) )  ! becomes cloud mixing ratio over land
    END DO
  else
    !if csurf/='EXTE', psea is not allocated
    DO JK=1,IKU
      ZRZ(JK,2)=TPFLYER%INTERP_HOR_FROM_MASSPOINT( PR(:,:,JK,2) )
      ZRZ(JK,7) = 0.
    END DO
  end if
  ALLOCATE(ZAELOC(IKU))
  ...
  ! apply attenuation
  ALLOCATE(ZZMZ(IKU))
  ZZMZ(:)=TPFLYER%INTERP_HOR_FROM_MASSPOINT( ZZM(:,:,:) )
  ! nadir
  ZAETOT=1.
...
IF (SIZE(PTKE)>0) TPFLYER%XTKE  (ISTORE) = TPFLYER%INTERP_FROM_MASSPOINT( PTKE )
IF (SIZE(PTS) >0) TPFLYER%XTSRAD(ISTORE) = TPFLYER%INTERP_HOR_FROM_MASSPOINT(PTS )
