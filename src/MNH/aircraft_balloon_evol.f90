...
END DO
TPFLYER%XRTZ  (ISTORE,:) = TPFLYER%INTERP_FROM_MASSPOINT_VERT(ZR(:,:,:))
DO JLOOP=1,SIZE(PR,4)
  TPFLYER%XRZ  (ISTORE,:,JLOOP) = TPFLYER%INTERP_FROM_MASSPOINT_VERT(PR(:,:,:,JLOOP))
END DO

TPFLYER%XFFZ  (ISTORE,:) = TPFLYER%INTERP_FROM_MASSPOINT_VERT(SQRT(PU**2+PV**2))


!PW:TODO: n'allouer et ne transferer que si a 1 sens!



IF (CCLOUD=="LIMA") THEN
  TPFLYER%XCIZ  (ISTORE,:) = TPFLYER%INTERP_FROM_MASSPOINT_VERT(PSV(:,:,:,NSV_LIMA_NI))
  TPFLYER%XCCZ  (ISTORE,:) = TPFLYER%INTERP_FROM_MASSPOINT_VERT(PSV(:,:,:,NSV_LIMA_NC))
  TPFLYER%XCRZ  (ISTORE,:) = TPFLYER%INTERP_FROM_MASSPOINT_VERT(PSV(:,:,:,NSV_LIMA_NR))
ELSE IF ( CCLOUD=="ICE3" .OR. CCLOUD=="ICE4" ) THEN
  TPFLYER%XCIZ  (ISTORE,:) = TPFLYER%INTERP_FROM_MASSPOINT_VERT(PCIT(:,:,:))
END IF
...
IF (CCLOUD=="LIMA" .OR. CCLOUD=="ICE3" ) THEN ! only for ICE3 and LIMA
  TPFLYER%XLWCZ  (ISTORE,:) = TPFLYER%INTERP_FROM_MASSPOINT_VERT((PR(:,:,:,2)+PR(:,:,:,3))*PRHODREF(:,:,:))
  TPFLYER%XIWCZ  (ISTORE,:) = TPFLYER%INTERP_FROM_MASSPOINT_VERT((PR(:,:,:,4)+PR(:,:,:,5)+PR(:,:,:,6))*PRHODREF(:,:,:))
  ZTEMPZ(:)=TPFLYER%INTERP_FROM_MASSPOINT_VERT(PTH(II_M:II_M+1,IJ_M:IJ_M+1,:) * ZEXN(:,:,:))
  ZRHODREFZ(:)=TPFLYER%INTERP_FROM_MASSPOINT_VERT(PRHODREF(:,:,:))
  IF (CCLOUD=="LIMA") THEN
    ZCCI(:)=TPFLYER%INTERP_FROM_MASSPOINT_VERT(PSV(:,:,:,NSV_LIMA_NI))
    ZCCR(:)=TPFLYER%INTERP_FROM_MASSPOINT_VERT(PSV(:,:,:,NSV_LIMA_NR))
    ZCCC(:)=TPFLYER%INTERP_FROM_MASSPOINT_VERT(PSV(:,:,:,NSV_LIMA_NC))
  ELSE
    ZCIT(:)=TPFLYER%INTERP_FROM_MASSPOINT_VERT(PCIT(:,:,:))
  ENDIF
  DO JLOOP=3,6
    ZRZ(:,JLOOP)=TPFLYER%INTERP_FROM_MASSPOINT_VERT(PR(:,:,:,JLOOP))
  END DO
  if ( csurf == 'EXTE' ) then
    DO JK=1,IKU
  ...
  ! apply attenuation
  ALLOCATE(ZZMZ(IKU))
  ZZMZ(:)=TPFLYER%INTERP_FROM_MASSPOINT_VERT(ZZM(:,:,:))
  ! nadir
  ZAETOT=1.
...
! vertical wind
TPFLYER%XWZ  (ISTORE,:) = TPFLYER%INTERP_FROM_MASSPOINT_VERT(ZWM(:,:,:))
