...
CONTAINS
  ...
  SUBROUTINE CARTESIAN(TPROC,NB_PROC,X_DIM,Y_DIM,X_DOMAINS,Y_DOMAINS)
    ...
    !*       0.1   declarations of arguments
    !
    TYPE(ZONE_LL), DIMENSION(:),TARGET  :: TPROC
    ...
    INTEGER :: SOMX,SOMY,INTX,INTY,IDOM,II,IJ
  ...
  !
  !     #################################################################
  SUBROUTINE SPLIT2(X_DIM,Y_DIM,Z_DIM,NB_PROC,TPROC,HSPLITTING, KX_DOMAINS,KY_DOMAINS)
    !     #################################################################
    !
