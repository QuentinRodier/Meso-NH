       ...
       ELSE ! multiprocesses execution & // IO
         ...
         DO JKK=1,SIZE(PFIELD,3) ! IKU_ll
            ...
            IF (ISP == IK_RANK )  THEN 
               ...
               !
               IF (GLFI) CALL IO_WRITE_FIELD_LFI(TPFILE,TPFIELD,ZSLICE_ll,IRESP_TMP,KVERTLEVEL=JKK,KZFILE=IK_FILE+1)
               IF (GNC4) CALL IO_WRITE_FIELD_NC4(TPFILE,TPFIELD,ZSLICE_ll,IRESP_TMP,KVERTLEVEL=JKK,KZFILE=IK_FILE+1)
...
! end of MNH_GA
#endif
       CALL MPI_ALLREDUCE(-ABS(IRESP_ISP),IRESP_TMP,1,MPI_INTEGER,MPI_MIN,TPFILE%NMPICOMM,IRESP)   
