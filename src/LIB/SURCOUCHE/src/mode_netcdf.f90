...
INTERFACE IO_WRITE_FIELD_NC4
   MODULE PROCEDURE IO_WRITE_FIELD_NC4_X3
...
SUBROUTINE IO_WRITE_ATTR_NC4(TPFIELD,KNCID,KVARID)
!
...
IF(TPFIELD%NGRID<0) THEN
  PRINT *,'WARNING: IO_WRITE_ATTR_NC4: TPFIELD%NGRID not set'
ENDIF
...
IF(LEN_TRIM(TPFIELD%CCOMMENT)==0) THEN
  PRINT *,'WARNING: IO_WRITE_ATTR_NC4: TPFIELD%CCOMMENT not set'
ENDIF
...
IF(LEN_TRIM(TPFIELD%CSTDNAME)==0) THEN
  PRINT *,'WARNING: IO_WRITE_ATTR_NC4: TPFIELD%CSTDNAME not set'
ENDIF
...
IF(LEN_TRIM(TPFIELD%CUNITS)==0) THEN
  PRINT *,'WARNING: IO_WRITE_ATTR_NC4: TPFIELD%CUNITS not set'
ENDIF
STATUS = NF90_PUT_ATT(KNCID, KVARID,'units', TRIM(TPFIELD%CUNITS))
IF (STATUS /= NF90_NOERR) CALL HANDLE_ERR(status,__LINE__,'IO_WRITE_ATTR_NC4 [NF90_PUT_ATT]')
!
END SUBROUTINE IO_WRITE_ATTR_NC4
...
SUBROUTINE IO_WRITE_FIELD_NC4_X3(TPFIELD,PZCDF,KRESP)
!
USE MODD_FM, ONLY : FMHEADER
USE MODD_IO_ll, ONLY : TFIELDDATA
!
...
IF (STATUS /= NF90_NOERR) THEN
   ! Get the netcdf dimensions
   CALL FILLVDIMS(PZCDF, INT(SHAPE(TPFIELD%XFIELDDATA3D),KIND=IDCDF_KIND), TPFIELD%CDIR, IVDIMS)

   ! Define the variable 
   STATUS = NF90_DEF_VAR(INCID, YVARNAME, NF90_DOUBLE, IVDIMS, IVARID)
   IF (status /= NF90_NOERR) CALL HANDLE_ERR(status,__LINE__,'IO_WRITE_FIELD_NC4_X3[NF90_DEF_VAR]')
   CALL IO_WRITE_ATTR_NC4(TPFIELD,INCID,IVARID)
ELSE
   PRINT *,'IO_WRITE_FIELD_NC4_X3 : ', TRIM(YVARNAME), ' already defined !'
END IF

! Write the data
STATUS = NF90_PUT_VAR(INCID, IVARID, TPFIELD%XFIELDDATA3D)
...
!
SUBROUTINE IO_WRITE_FIELD_NC4(A,B,C)
INTEGER :: A,B,C
