...
!!$    IF(LWEST_ll(J)) THEN
    IF(TPPROCONF%TBOUND(J)%WEST) THEN
...
!
!!$    IF(LSOUTH_ll(J)) THEN
    IF(TPPROCONF%TBOUND(J)%SOUTH) THEN
...
!
!!$    IF(LEAST_ll(J)) THEN
    IF(TPPROCONF%TBOUND(J)%EAST) THEN
...
!
!!$    IF(LNORTH_ll(J)) THEN
    IF(TPPROCONF%TBOUND(J)%NORTH) THEN
...
!
!     ######################################################
      SUBROUTINE INI_CYCLIC( TPPROCONF, &
                             TPSENDHALO, TPRECVHALO, &
...
!
!!$    IF(LWEST_ll()) THEN
    IF(TPPROCONF%TBOUND(IP)%WEST) THEN
!
      DO J = 1, NPROC
!
...
!
!!$    IF(LEAST_ll()) THEN
    IF(TPPROCONF%TBOUND(IP)%EAST) THEN
!
      DO J = 1, NPROC
!
...
!
!!$    IF(LSOUTH_ll()) THEN
    IF(TPPROCONF%TBOUND(IP)%SOUTH) THEN
!
      DO J = 1, NPROC
!
...
!
!!$    IF(LNORTH_ll()) THEN
    IF(TPPROCONF%TBOUND(IP)%NORTH) THEN
!
      DO J = 1, NPROC
!
...
!!$    IF( LEAST_ll() .AND. LSOUTH_ll() ) THEN
    IF( TPPROCONF%TBOUND(IP)%EAST .AND. TPPROCONF%TBOUND(IP)%SOUTH ) THEN
!
      DO J = 1, NPROC
!
!       list all the subdomains and select the NORTH_WEST one
!!$        IF( LNORTH_ll(J) .AND. LWEST_ll(J) ) THEN
        IF( TPPROCONF%TBOUND(J)%NORTH .AND. TPPROCONF%TBOUND(J)%WEST ) THEN
!
...
!!$    IF( LNORTH_ll() .AND. LWEST_ll() ) THEN
    IF( TPPROCONF%TBOUND(IP)%NORTH .AND. TPPROCONF%TBOUND(IP)%WEST ) THEN
!
      DO J = 1, NPROC
!
!       list all the subdomains and select the SOUTH-EAST one
!!$        IF( LEAST_ll(J) .AND. LSOUTH_ll(J) ) THEN
        IF( TPPROCONF%TBOUND(J)%EAST .AND. TPPROCONF%TBOUND(J)%SOUTH ) THEN
!
!         we add the zone to the send correspondant
          TZZONE_INTER = ZONE_ll( J, 3, &
                     TPPZS(IP)%NXOR,&
...
!!$    IF( LNORTH_ll() .AND. LEAST_ll() ) THEN
    IF( TPPROCONF%TBOUND(IP)%NORTH .AND. TPPROCONF%TBOUND(IP)%EAST ) THEN
!
      DO J = 1, NPROC
!
!       list all the subdomains and select the SOUTH-WEST one
!!$        IF( LSOUTH_ll(J) .AND. LWEST_ll(J) ) THEN
        IF( TPPROCONF%TBOUND(J)%SOUTH .AND. TPPROCONF%TBOUND(J)%WEST ) THEN
!
!         we add the zone to the send correspondant
...
!!$    IF( LSOUTH_ll() .AND. LWEST_ll() ) THEN
    IF( TPPROCONF%TBOUND(IP)%SOUTH .AND. TPPROCONF%TBOUND(IP)%WEST ) THEN
!
      DO J = 1, NPROC
!
!       list all the subdomains and select the NORTH-EAST one
!!$        IF( LNORTH_ll(J) .AND. LEAST_ll(J) ) THEN
        IF( TPPROCONF%TBOUND(J)%NORTH .AND. TPPROCONF%TBOUND(J)%EAST ) THEN
!
!         we add the zone to the send correspondant
          TZZONE_INTER = ZONE_ll( J, 5, &
            TPPZS(IP)%NXOR,&
...
!!$  IF (LWEST_ll() .AND. LEAST_ll() .AND. CLBCX(ICURMODEL, 1)=='CYCL') THEN
  IF (TPPROCONF%TBOUND(IP)%WEST .AND. TPPROCONF%TBOUND(IP)%EAST .AND. CLBCX(ICURMODEL, 1)=='CYCL') THEN
    ICOUNTWEST = 1
    ICOUNTEAST = 1
!
    IWEST(1) = IP
...
!!$  IF (LNORTH_ll() .AND. LSOUTH_ll() .AND. CLBCY(ICURMODEL, 1)=='CYCL') THEN
  IF (TPPROCONF%TBOUND(IP)%NORTH .AND. TPPROCONF%TBOUND(IP)%SOUTH .AND. CLBCY(ICURMODEL, 1)=='CYCL') THEN
!
    ICOUNTNORTH = 1
    ICOUNTSOUTH = 1
!
...
!
  ...
  CALL COMPUTE_CRSPD_MAX(KMAXSIZE,TPCOMDATA%TSEND_SXP1_YP2_Z_SX_YP2_ZP1)
  CALL COMPUTE_CRSPD_MAX(KMAXSIZE,TPCOMDATA%TRECV_SXP1_YP2_Z_SX_YP2_ZP1)
  CALL COMPUTE_CRSPD_MAX(KMAXSIZE,TPCOMDATA%TSEND_SXP2_Y_ZP1_SXP1_YP2_Z)
  CALL COMPUTE_CRSPD_MAX(KMAXSIZE,TPCOMDATA%TRECV_SXP2_Y_ZP1_SXP1_YP2_Z)
!JUAN ZSPLITTING
!
!-------------------------------------------------------------------------------
!
...
!              ------------------------------------------
!
  CALL INI_CYCLIC2( TPPROCONF, &
                    TPCOMDATA%TSEND_HALO2, &
...
!
!     ###############################################
      SUBROUTINE INI_CYCLIC2( TPPROCONF, TPSEND, TPRECV, TPPZS )
!     ###############################################
!
